...
my $wallet_name = "";  # keosd wallet name  钱包名称
my $pwd_file = "";     # password file      钱包密码文件名称
my $bridge_account = "";#bridge accounts    中转账号

my $pos =  $ARGV[0];     #启动值要选择最近已经确认的区块，并且已经账号注册完成的编号。
 ...
 { 
      ...
      if($get_actions[1]=~ /"from":"(.+)","to":"(.+)","quantity":"(.+)\s+EOS","memo":"(.*)"/)  #匹配转入金额和备注
      {
	    ...
	    `cat $pwd_file |cleos wallet unlock -n $wallet_name`;
         while()
         {
    	  my @reg_back = `cleos transfer $bridge_account accountspapa "$send EOS" "$memo" 2>&1`;  #注册
          if ($reg_back[0] =~ /Error\s+3080004|3080002|3081001/ ) #cpu或者net资源不足错误,或者deadline
          {
            ...
            my @powerup_back =`cleos push action eosio powerup '["$bridge_account","$bridge_account","1","111460","267858576","0.0090 EOS"]' -p $bridge_account\@active 2>&1`;
